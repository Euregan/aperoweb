...
const Talks = () => {
  ...
  const loadingCalendar = [...Array(6).keys()].map(() => <LoadingCard lines={5}/>).concat([...Array(6).keys()].map(() => <LoadingCard lines={3}/>))
  ...
  return (
    <div>
      <Layout>
        <h2>Calendar</h2>
        <Grid>
          {calendar === 'loading' ? loadingCalendar : calendar.map((talk, index) => talk
        ...
        <h2>Not planned</h2>
        <Grid>
          {noDate === 'loading' ? <LoadingCard/> : noDate.map((talk, index) => (<Talk key={index} talk={talk}/>))}
        </Grid>
      </Layout>
    </div>
  )
