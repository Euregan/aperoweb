...
const Home = () => {
  ...
  if (talks === 'loading' && nextTalk === 'loading') {
    ...
    talks.then(setTalks)
    talks.then(talks => talks.find(talk => talk && new Date(talk.date) > new Date()))
      .then(setNextTalk)
  }

  if (tweets === 'loading' && nextTweet === 'loading') {
    const tweets = fetch('/tweets')
      .catch(error => console.error(error) || [])

    tweets.then(setTweets)
    tweets.then(tweets => tweets.find(tweet => new Date(tweet.date) > new Date()))
      .then(setNextTweet)
  }

  const nextTalkCard = nextTalk === 'loading'
  ...
  const plannedTalksCard = talks === 'loading'
    ? <LoadingCard title="Planned talks" lines={5}/>
    : <Card id="plannedTalks" title="Planned talks">
        {talks.filter(talk => talk && talk.date).map(talk => (<div>{talk.name}</div>))}
      </Card>
